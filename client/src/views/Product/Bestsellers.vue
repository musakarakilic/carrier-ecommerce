        ...
        <div class="flex flex-col lg:flex-row gap-8">
          ...
          <div class="lg:w-3/4">
            ...
            <div v-else class="text-center py-12 bg-gray-50 rounded-xl">
              ...
              <p class="mt-2 text-gray-500">There are currently no products to display in this category.</p>
            </div>
...
const loadBestsellerProducts = async () => {
  try {
    ...
    // Get products sorted by sales from API
    // API already returns products sorted by sales
    const response = await productService.getBestsellerProducts({
      limit: 100, // Request more products, use API's own sorting
    ...
    if (response && response.products) {
      ...
      // Save all products - already sorted by sales from API
      ...
      console.log(`Total ${allProducts.value.length} products loaded, showing first ${products.value.length} products.`);
    } else {
...
// Change page (no longer used but kept for old code)
const changePage = (page) => {
  if (page >= 1 && page <= totalPages.value) {
    currentPage.value = page;
    // Now using scroll-based loading instead of loadBestsellerProducts
  }
};
...
// When page is loaded
onMounted(async () => {
  console.log('BestsellerProductsView component loaded - AFTER ROUTER FIX');
  document.title = 'Bestselling Products';
...
const handleFilters = (newFilters) => {
  ...
  loadBestsellerProducts(); // Reload with filters
};

const clearFilters = () => {
  filters.value = {};
  loadBestsellerProducts(); // Clear filters and reload
};
